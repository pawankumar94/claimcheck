---
name: claimcheck
description: Pin assertions about this repo to the files they cite, list tickets before editing a path, and re-check when those files change. Do not leave constraints only in chat.
---

# claimcheck: file-bound tickets
...
1. **Pin assertions to files.** After you state a constraint about the repo,
   write a ticket: `claimcheck pin --text "..." --file src/a.ts` (or call
   `pin_claim`). Do not leave it only in the transcript.

2. **Look before you edit.** Before editing a path, list tickets on it
   (`claimcheck status src/a.ts` or `list_claims_for_file`). Honor open and
...
write `.claims/<id>.json` with `text` and `files`, and read that folder
...
claimcheck check auth-only-through-middleware
```

MCP: `pin_claim`, `list_claims_for_file`, `check_claim`.

Config A/B (`claimcheck measure`) is the lab for when the claim itself is
