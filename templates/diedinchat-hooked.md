...
1. **Pin assertions to files, with evidence.** After you state a constraint
   ...
   Check first that an existing rule does not already say it — `pin` will refuse
   a duplicate, and a near-duplicate is noise:
...
2. **A pre-write hook is installed. Do not look up rules yourself.**
   Before every Write, Edit or Delete, the hook resolves the rules covering that
   path and puts them in front of you automatically. Honor what it shows you.

   - **Do not** run `diedinchat status` before a normal edit. It is already done.
   - **Do not** re-read a file just to check for rules.
   - **Do not** run `diedinchat check` after an ordinary edit. Run it only when
     your change touched a rule's frozen evidence, or when asked.

   A write that breaks a rule is denied by the hook with the reason, so you do
   not need to verify anything in advance.
